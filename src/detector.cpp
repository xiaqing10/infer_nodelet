...
int YoloV8_det::Detect(const std::vector<bm_image>& input_images, std::vector<YoloV8BoxVec>& boxes, int index=0) {
    ...
    //     // 保存bm_image 到本地
    //     static int count = 0;
    //     bm_image save_bm_image = input_images[0];
    //     std::string  save_bmp = "/home/files/tmp/out/" + std::to_string(index) + "_" + std::to_string(count)  + ".bmp";
    //     bm_image_write_to_bmp(save_bm_image, save_bmp.c_str());
    //     count ++;
    // }
...
int YoloV8_det::pre_process(const std::vector<bm_image>& images, 
    ...
    // for(int i = 0; i < 1; i++){
    //     std::cout << m_converto_imgs.data() << std::endl;
    // }
...
int YoloV8_det::forward(bm_tensor_t& input_tensor, std::vector<bm_tensor_t>& output_tensors){
    ...
    // static int count = 0;
    // std::ifstream input_data("/home/files/tmp/out/"+std::to_string(count)+".bin", std::ios::binary);
    // static float *input = new float[3*720*1080];
    // input_data.read((char*)input, 3*720*1080*sizeof(float));
    // bm_memcpy_s2d(handle, input_tensor.device_mem, input);

    // float* data_input = NULL;
    // bm_tensor_t tensor_box;
    // if(input_tensor.shape.num_dims == 4){
    //     data_input = get_cpu_data(&input_tensor, 1);
    //     std::cout << "input:  << "  << std::endl;
    //     for(int _i =1; _i < 640*640*3 ; ) {
    //         if (data_input[_i] < 0.44 || data_input[_i] > 0.45){
    //             std::cout << " " << data_input[_i] ;
    //             _i *= 2;
    //         }
    //         else _i += 4;
    ...
    //     }
    //     std::cout << " " << std::endl;
    //     std::cout << "input min: " <<  (*std::min_element(data_input, data_input + 640*640*3))<< "  max: " << (*std::max_element(data_input, data_input + 640*640*3)) << std::endl;
...
int YoloV8_det::post_process(const std::vector<bm_image>& input_images, 
    ...
    for(int i = 0; i < output_tensors.size(); i++) {
        if(output_tensors[i].shape.num_dims == 3){
            ...
            // std::cout <<" output: >> "  << netinfo->output_scales[i] << std::endl;
            // for(int _i =1; _i < 14*8400 ; ) {
            //     std::cout << " " << data_box[_i] ;
            //     _i *= 3;
            // }
            // std::cout << " " << std::endl;
            // std::cout << "output min: " <<  (*std::min_element(data_box, data_box + 1*14*8400))<< "  max: " << (*std::max_element(data_box, data_box + 1*14*8400)) << std::endl;
