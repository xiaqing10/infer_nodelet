...
int YoloV8_det::post_process(const std::vector<bm_image>& input_images, 
    ...
    for (int batch_idx = 0; batch_idx < input_images.size(); ++batch_idx) {
        ...
        for (int i = 0; i < box_num; i++) {
            ...
            float* cls_conf = batch_data_box + box_index + 4 * offset; //output_tensor's last dim: [x, y, w, h, cls_conf0, ..., cls_conf14, rotate_angle]
            ...
            // best class
            YoloV8Box box;
            box.class_id = argmax(batch_data_box + box_index + 4 * offset, m_class_num, offset);
            box.score = batch_data_box[box_index + 4 * offset + box.class_id * offset];
...
int YoloV8_det::argmax(float* data, int num, int stride) {
    float max_value = 0.0;
    int max_index = 0;
    for (int i = 0; i < num; ++i) {
        float value = data[i * stride];
        if (value > max_value) {
            max_value = value;
            max_index = i;
        }
